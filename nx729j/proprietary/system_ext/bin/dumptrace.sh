...
# zte avoid dup self running
# get the self path
file_name=$0
exec_cnt=`pgrep -c -f $file_name`
# if pgrep not exist, then a error reported $file_name: unexpected '$', but no matter
if [ "$exec_cnt" -gt "1" ]; then
    echo "another dumptrace is in process, skip"
    exit 0
fi
...
if [ $? -eq 0 ]; then
    ...
    if [ -d /d/binder ];then
        ...
        tar zcf $file.tar.gz $trace_file_name events.txt mainsystem.txt dumpsys_log.txt
    fi
